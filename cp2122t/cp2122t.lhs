...
%
\just\equiv{ |inNEList := either singl cons| }
...
\just\equiv{ Natural-id, lei(1) ; Igualdade Extensional, lei(71) }
        |lcbr(
          i1 x = (outNEList . singl) x
     )(
          i2 (x,y) = (outNEList . cons) (x,y)
     )|
%
...
&
    |A + (LTree A)|^2
          \ar[ll]_-{|inLTree = either Leaf Fork|}
\\
     A^*
          \ar[u]^-{list2LTree}
          \ar[r]_-{|outNEList|}
...
&
     |A + A|^* |>< A|^*
          \ar[u]_{|id + list2LTree|^2}
}
\end{eqnarray*}

Nós sabemos que $g\_list2LTree = id + |split f g|$. Então sobra determinar $f$ e $g$. Para tal vamos analisar
cada função individualmente.

A função $f$ vai processar o $tuple$ $A |><| A^*$ de maneira a devolver o seu primeiro elemento à cabeça da primeira 
...
Como só queremos a primeira pare do $tuple$ segue de forma composta a função $|p1|$ para isolar esse elemento obtendo 
$|p1 . half|$. Como queremos preservar o primeiro elemento para o adicionar à cabeça da lista não lhe
fazemos nada aplicando-lhe a função $|id|$ obtendo $|id >< (p1 . half)|$. Para terminar como $f$ retorna uma lista
temos que a contruir utilizando o $tuple$ e para tal fazemos uso do contrutor $|cons|$ obtendo 
$|cons . (id >< (p1 . half))|$.
...
&
    |A + (LTree A)|^2
           \ar[d]^{|id + lTree2MTree|^2}
           \ar[l]_-{|inLTree = either Leaf Fork|}
\\
     |FTree Z (Z >< A)|
&
     |A + (FTree Z (Z >< A))|^2
...
Nós sabemos que $|g_lTree2MTree = either g1 g2|$. Então sobra determinar $|g1|$ e $|g2|$. Para tal vamos analisar
cada função individualmente.

A função $|g1|$ vai tratar do caso em que estamos a lidar com uma folha da $|LTree|$ do $input$. Esta é bastante 
...
g_pairsList = (id -|- split auxPairList ((drop 1) . p2)) . outList
...
\begin{code}
pairL :: [a] -> [(a,a)]
pairL = pairLAux . anaList g where
  g = (id -|- split auxPairL p2) . outList
